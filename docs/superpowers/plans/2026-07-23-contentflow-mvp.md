...
- [ ] Write failing tests for register/login and JWT rejection.
- [ ] Run `mvn test` in `backend/` and confirm tests fail because implementation is missing.
- [ ] Implement Spring Boot app, security config, JWT service, auth DTOs, user mapper, and auth service.
- [ ] Add `schema.sql` for `sys_user`, `cf_project`, `cf_document`, and `cf_content`.
- [ ] Run `mvn test` in `backend/` and confirm auth tests pass.
...
- [ ] Write failing tests for project owner isolation.
- [ ] Write failing tests for unsupported document type returning `400`.
- [ ] Write failing tests for successful generation saving `cf_content`.
- [ ] Run `mvn test` in `backend/` and confirm expected failures.
- [ ] Implement project mapper/service/controller.
- [ ] Implement document mapper/service/controller and Agent indexing client.
- [ ] Implement content mapper/service/controller and Agent generation client.
- [ ] Implement internal project context and internal content save APIs.
- [ ] Run `mvn test` in `backend/` and confirm all backend tests pass.
- [ ] Commit backend business APIs.
...
- [ ] Write failing pytest for indexing `.txt` text and retrieving only matching `projectId`.
- [ ] Run `pytest` in `agent/` and confirm the retrieval test fails.
- [ ] Implement Pydantic schemas and Chroma retrieval service.
- [ ] Implement `/documents/index` endpoint.
- [ ] Run `pytest` in `agent/` and confirm retrieval tests pass.
- [ ] Commit Agent retrieval foundation.
...
- [ ] Write failing pytest for graph node order and structured output.
- [ ] Run `pytest` in `agent/` and confirm graph test fails.
- [ ] Implement LangGraph nodes: `parse_request`, `retrieve_context`, `generate_content`, `format_output`.
- [ ] Implement LLM adapter using environment model config with deterministic fallback when no API key is present.
- [ ] Implement `/generate` endpoint.
- [ ] Run `pytest` in `agent/` and confirm all Agent tests pass.
- [ ] Commit Agent LangGraph generation.
...
- [ ] Write failing Vitest tests for auth token persistence and logout.
- [ ] Run `npm test` in `web/` and confirm expected failures.
- [ ] Implement Vite, Vue, Pinia, Router, Axios client, auth store, and login/register view.
- [ ] Run `npm test` in `web/` and confirm frontend auth tests pass.
...
- [ ] Write failing Vitest tests for project API client path construction and generation loading state.
- [ ] Run `npm test` in `web/` and confirm expected failures.
- [ ] Implement project list view.
- [ ] Implement project detail view with document upload.
- [ ] Implement generation view.
- [ ] Implement content detail view.
- [ ] Run `npm test` in `web/` and confirm all frontend tests pass.
- [ ] Commit frontend workflow.
...
- [ ] Add placeholder-only env examples.
- [ ] Add startup commands for PostgreSQL-backed backend, Agent, and web.
- [ ] Add verification commands: `mvn test`, `pytest`, `npm test`, and health checks.
- [ ] Run available verification commands locally.
- [ ] Record any missing local runtime dependency clearly in final handoff.
